...
(function(w){
	...
	store.prototype = {
		...
		addDataPoint: function(x, y){
			...
			if(this.max < data[x][y]){
				...
				for(var one in data){
					for(var two in data[one]){
						heatmap.drawAlpha(one, two, data[one][two]/this.max);
					}
				}
			...
			heatmap.drawAlpha(x, y, 1); // for newly added point, set alpha to 1 in order to show
		},
		setDataSet: function(obj){
			this.max = obj.max;
			var heatmap = this.get("heatmap"),
			data = this.get("data"),  // data already be cleared when getting here!!!
			d = obj.data,
			dlen = d.length;
			
			while(dlen--){
				var point = d[dlen];
				// re-adjust count based on current max
				if(point.count > this.max){
					point.count = this.max;
				}
				heatmap.drawAlpha(point.x, point.y, point.count/this.max);
	...
	heatmap.prototype = {
		...
		drawAlpha: function(x, y, alpha){   // caller pass alpha[0..255] in
				if(isNaN(parseInt(x)) || isNaN(parseInt(y)) || isNaN(parseInt(alpha))){
					return;   // good fix!!!
				}
        ...
        fadeOut: function(){
				...
				this.get("actx").clearRect(0,0,this.get("width"),this.get("height"));
				...
				for(var one in data){					
					...
					for(var two in data[one]){
						if(isNaN(parseInt(two)))
							continue;
					  // do not know why function include/any get added to data[][] array!!!
					  if(typeof data[one][two] == 'function')  
					    continue;

					  data[one][two] = data[one][two]/2;  // half the count
					  this.drawAlpha(one, two, data[one][two]/this.store.max);
					}
				}
				// after fadeOut, reduce max count
				if(this.store.max > 0){
					this.store.max -= 1;
				}
        },
