...
HeatmapOverlay.prototype.onAdd = function(){
	...
	el.style.cssText = "position:absolute;top:0;left:0;width:"+window.innerWidth+"px;height:"+window.innerHeight+"px;overflow-x:hidden;overflow-y:hidden;";

	this.conf.element = el;
	this.conf.width = window.innerWidth;
	this.conf.height = window.innerHeight;

	var panes = this.getPanes();
	panes.overlayLayer.appendChild(el);

	console.log('div offsetWidth:' + el.offsetWidth + ' winW:' + window.innerWidth);
...
HeatmapOverlay.prototype.pixelTransformWrap = function(p){
...
HeatmapOverlay.prototype.addDataPoint = function(lat, lng, count){
	...
	console.log('OverlayView addDataPoint:' + point.x + ':' + point.y + ':' + count);
	this.heatmap.store.addDataPoint(point.x, point.y, count);
	this.latlngs.push({latlng: latlng, c: count});
}

HeatmapOverlay.prototype.fadeOut = function(){
	this.heatmap.fadeOut();
}
